...
const FileDownloads = () => {
  ...
  // FETCH ADMIN-SENT FILES FROM FIREBASE
  useEffect(() => {
    ...
    console.log("Fetching admin-sent files for user:", {
      uid: currentUser.uid,
      email: currentUser.email
    });

    // Query to find admin-sent files
    const queries = [
      ...
      const unsubscribe = onSnapshot(
        ...
        (snapshot) => {
          const docs = snapshot.docs
            ...
            // Filter to show only admin-uploaded files
            .map(doc => ({
              ...doc,
              files: (doc.files || []).filter(file => 
                file.adminUploaded && file.adminFile // Admin uploads only
              )
            }))
            // Remove documents that have no admin-uploaded files
            .filter(doc => doc.files.length > 0);
          
          console.log(`Query ${index} found ${docs.length} admin-sent documents`);
    ...
    const fetchAllDocuments = () => {
      ...
      const unsubscribe = onSnapshot(
        ...
        (snapshot) => {
          ...
          // Filter for current user and only admin-sent files
          ...
          console.log(`Found ${userDocs.length} admin-sent documents after filtering`);
  ...
  // Calculate total files and total size (only admin-sent)
  const totalFiles = fileDocs.reduce((sum, doc) => sum + (doc.files?.length || 0), 0);
  const totalSize = fileDocs.reduce((sum, doc) => {
    const adminFilesSize = (doc.files || []).reduce((fileSum, file) => fileSum + (file.fileSize || 0), 0);
    return sum + adminFilesSize;
  }, 0);
  ...
  const allFiles = fileDocs.flatMap((doc) =>
    (doc.files || []).map((file) => ({
      ...file,
      // Use adminFile URL for downloads
      downloadURL: file.adminFile,
      parentDocId: doc.id,
  ...
  console.log("Total admin-sent files to display:", allFiles.length);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="text-center">
          ...
          <p className="ml-3 text-gray-700 mt-3">Loading files sent to you...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gray-50 p-6 md:p-10">
        <div className="max-w-5xl mx-auto">
          <div className="bg-white p-20 text-center rounded-xl border-2 border-dashed">
            ...
            <p className="text-gray-400">You need to be logged in to access files sent to you</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* HEADER */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Files Sent to You</h1>
          <p className="text-gray-600 mt-2">Files uploaded by admin for you</p>
        </div>
        ...
        {fileDocs.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-white p-4 rounded-lg border shadow-sm">
              <div className="text-sm text-gray-500">Files Sent to You</div>
        ...
        {filteredFiles.length === 0 ? (
          <div className="bg-white p-16 text-center rounded-xl border-2 border-dashed border-gray-300">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">
              {searchTerm ? "No files match your search" : "No files sent to you yet"}
            </h3>
            <p className="text-gray-500">
              {searchTerm 
                ? "Try a different search term" 
                : "Admin will upload files for you here"}
            </p>
          </div>
        ) : (
          <div>
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Files Sent to You ({filteredFiles.length})</h2>
