...
const ProductCard = ({ product, addToCart, getQuantity, updateQuantity, onToggleWishlist, isLoggedIn, location }) => {
  ...
  const handleWishlistToggle = (e) => {
    ...
    if (!isLoggedIn) {
      ...
      // Pass state to login page including product info
      navigate("/login", { 
        state: { 
      ...
      // Show toast message
      if (onToggleWishlist) {
        onToggleWishlist("⚠️ Please login to add to wishlist");
      }
      return;
    }
    
    console.log("User is logged in, toggling wishlist...");
    // If logged in, toggle wishlist
    const success = toggleWishlist(product);
    console.log("toggleWishlist returned:", success);
    
    // ✅ CHECK if toggle was successful (returns false if not logged in)
    if (success) {
      // if (onToggleWishlist) {
      ...
      // This should not happen since we already checked isLoggedIn above
      // But just in case as a fallback
      console.log("toggleWishlist failed, redirecting to login...");
      navigate("/login", { 
        state: { 
          from: location.pathname,
          wishlistRedirect: true,
