...
const SellProductForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    contactNumber:
      editDoc?.contactNumber || editDoc?.seller?.contactNumber || "",
  ...
  const [isSummaryPanelOpen, setIsSummaryPanelOpen] =
    useState(initialSummaryOpen);
  ...
  const handleChange = (e) => {
    ...
    if ((name === "price" || name === "offerPrice") && Number(value) < 0)
      return;
  ...
  const saveCustomerDetails = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, CUSTOMER_COLLECTION, user.uid), base, {
        merge: true,
      });
      await setDoc(doc(db, USERS_COLLECTION_MIRROR, user.uid), base, {
        merge: true,
      });
      setCustomerDoc(base);
    } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (step < 2) {
      ...
      const offerPrice =
        formData.offerPrice !== "" ? Number(formData.offerPrice) : null;
    ...
    try {
      ...
      const sellerSnapshot = {
        ...
        displayName: user.displayName || customerDoc?.displayName || "",
        contactNumber:
          formData.contactNumber || customerDoc?.contactNumber || "",
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const safeExt = ["jpg", "jpeg", "png", "webp"].includes(ext)
          ? ext
          : "jpg";
        const path = `${COLLECTION}/${user.uid || "anon"}/products/${
          isEdit ? editDoc.id : "new"
        }/${Date.now()}_${i}.${safeExt}`;
      ...
      if (isEdit) {
        const updatePayload = {
          ...
          offerPrice:
            formData.offerPrice === "" ? null : Number(formData.offerPrice),
    ...
    return (
      <div className="bg-white p-8 rounded-2xl shadow-xl border border-red-300 w-full max-w-md text-center">
        ...
        <p className="text-gray-600 mb-6">
          Please log in or sign up to access the Sell Product Form.
        </p>
        <button
          onClick={onCancel}
  ...
  const discountPct = hasDiscount
    ? Math.round(((priceNum - offerPriceNum) / priceNum) * 100)
    : 0;
  ...
  const allImages = [
    ...existingImages.map((url) => ({
      url,
      isExisting: true,
      isPrimary: url === existingImages[0],
    })),
    ...images.map((img) => ({
      url: img.preview,
      isExisting: false,
      isPrimary:
        existingImages.length === 0 && img.preview === images[0]?.preview,
    })),
  ];
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        placeholder={
                          formData.price ? String(formData.price) : "e.g. 1799"
                        }
                        min="0"
                        step="1"
                        className="w-full pl-8 pr-4 py-3 text-sm border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-3">
        {allImages.map((img, idx) => (
          <motion.div
            ...
            style={{
              borderColor: img.isPrimary ? "#3b82f6" : "#e5e7eb",
            }}
          >
            {/* Image section with object-contain to show full image */}
  ...
  const approveItem = async (id, approved) => {
    try {
      ...
      setItems((prev) =>
        prev.map((it) =>
          it.id === id
            ? { ...it, approved, status: approved ? "active" : "pending" }
            : it
        )
      );
    } catch (e) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!user ? (
          <div className="bg-white p-8 rounded-2xl shadow-xl border border-red-300 w-full max-w-md mx-auto text-center mt-20">
            ...
            <p className="text-gray-600 mb-6">
              Please log in to view your product listings.
            </p>
            <button
              onClick={onClose}
              className="w-full bg-red-500 hover:bg-red-600 text-white py-3 rounded-xl font-bold"
...
const Oldee = () => {
  ...
  const isAdmin = useMemo(() => {
    ...
    const isAdminByUid = adminUsers.some(
      (admin) => admin.uid === currentUser.uid
    );
    const isAdminByEmail = adminUsers.some(
 ...
 const openCreate = () => {
  ...
  setShowUpload(true); // ALWAYS open modal
};
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      ...
                      <span>
                        {p.seller?.displayName?.split(" ")[0] || "Seller"}
                      </span>
                    </div>
                  </div>
                </div>
